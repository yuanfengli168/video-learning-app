...
9. **上传限制与引导策略（2026-09-16 敲定）**：
   - **硬上限 15 个/天/付费用户**（FREE 本就不能传）。定义语义：按
     `created_at` 计"今天新传的"——**跨天不滚动**（昨天的 4 个拖到
     今天处理，不占今天的额度：公共排队延迟是系统因素，不应惩罚
     个人配额；23:50/00:10 边界各扣各天，罕见且无害）。
   - **第二道锚点：同时在处理上限 6 个未完成视频**（不分天）——防
     "每天灌满、机器上永远趴着几十个"的存量滥用；与 15/天（入场
     速率）互补，双闸比单日额度干净。
   - **软引导（四档，beta 不含 >2h 红档）**：空闲无提示 / <30min 队列
     位置+ETA / 30min-2h"⏳ 今天处理压力较大，你的视频预计 ~X 小时后
     完成。可以先传，也可以挑最想看的先传。"/ （>2h 红档延后）。
     设计哲学：提醒**公共拥挤度**而非"你传太多"——不指责但自然
     抑制灌量；比"剩余 N 次额度"心理暗示更健康。
   - **排队中取消**：queued 状态直接用既有 DELETE /videos/{id}
     （行+磁盘文件一起清），在课程页/视频页对 queued 视频显示 ✕
     删除按钮 + 软提醒文案教用户"不想等可以直接删"。**transcribing
     中不提供取消**（GPU 已在烧：丢弃=浪费，杀线程=危险）。
   - **个人 LLM 配额告警**：付费用户周用量 >80% 时上传处提示"本周
     AI 处理额度还剩 ~20%"（100/周的个人侧提醒，与公共档位互补）。

10. **公平调度规则（2026-09-16 敲定，用户点名场景：20 人排队）**：
    目标行为——"每个人的第一个视频排在整体最前面"，不让两个重度
    用户的全部视频霸占前排。**两遍扫描**（确定性、可测试）：
    - **第 1 遍**：只认领"名下 0 个视频正在处理"的用户 → 20 个用户
      依次拿到第一个槽（30 分钟内全员开工，第一个人人平等）；
    - **第 2 遍**：仍有空槽且队列未排空 → 按 FIFO 补位给任何人下一
      个视频（单用户独场时两槽都归他，保利用率）。
    - 关键细节：**已完成的不算占位**——A1 完成后 A2 即获得第 1 遍
      资格（否则退化为纯串行）。"正在处理中"指 status ∈
      (transcribing)（generating 走云端不占槽）。
    - ETA = 前方排队数 × 实测均值 ÷ 槽位数。

11. **admin 上传统计（2026-09-16 敲定，不开新页）**：扩展
    /admin/analytics 新增"Upload activity"卡片（与 Content structure
    并排，7/30 天窗口可切）：上传用户数、总量+日 sparkline、
    **mean / median 分离展示**（两者差值直接暴露"个别重度用户拖高
    均值"）、**max 单用户单日**（滥用探针）、top-5 每日上传者、
    p50/p95 队列等待时长（软提醒档位的真实依据）。另加**队列实时
    深度数字**（当前排队 N + 预计排空时间）作为驾驶舱仪表。
    数据全部现成（videos.created_at + events），零新表零新端点，
    查询模式复刻 get_llm_provider_usage。

12. **容量结论更新（2026-09-16，含吞吐实测修正）**：9/12 实测
    large-v3-turbo 转写 31-65s/视频（M1 Max 竞争态下），Mac Studio
    30-core GPU 上 2 槽 ≈ 70-100 视频/小时。**瓶颈重排：内存不是
    瓶颈**（缓存锁后模型全局一份 ~3GB），真约束 = GPU 带宽饱和 +
    in-process 稳定性 → **2 槽定版，不测 3**（20-30% 吞吐增益不换
    稳定性余量，launch 周事故成本最高）。100 付费用户的平衡容量
    结论不变（晚高峰吞吐 2 小时排空），上传上限 10→15 不影响该
    结论（上限只封异常行为的顶）。**磁盘成为 100 用户规模的新
    P2**：最坏 200GB/天累积，90 天归档节奏按正常用量设计——
    uploads 卷"接近满载"告警进 events 表（待办）。**LLM 才是每
    用户最贵资源**：3000/周 ≈ 静态算 20-25 付费用户，但 90% 自动
    降级 OpenAI 后边际成本每用户 ~$0.10-0.50/月——容量在 100 用户
    前不构成利润瓶颈，100+ 时第一个升级项是把聊天改异步。
...
> ⚠️ 注：本附录是**审计当天（9/12）的原始记录**，其中"Mac Studio 64GB"
> 一行是当时的误判——审计实际发生在开发机 MacBook Pro M1 Max 64GB 上，
> 生产机规格 9/16 已实测核实为 Mac Studio M2 Max / 30-core GPU / 32GB
> （见决策 #2 与 #8 的修正记录）。原始记录保留供时间线追溯，**以修正
> 记录为准**。

- ~~Mac Studio，**64GB RAM**~~ → 实为开发机 MacBook Pro M1 Max 64GB；
  生产机 Mac Studio **32GB**（9/16 实测，决策 #8）
