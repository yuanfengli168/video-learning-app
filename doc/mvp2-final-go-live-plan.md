...
| | **(a) Actual shipped** (4 commits `61f2d89`…`69e1a91`): `gunicorn>=23.0.0` dep, `gunicorn.conf.py` (4 workers × 2 threads, 60s timeout, graceful shutdown, max_requests recycling, `forwarded_allow_ips=*` for tunnel), rewrite `start.sh` (default gunicorn; `SERVER=uvicorn` fallback for dev), rewrite `stop.sh` to handle both, new `/api/ready` endpoint (k8s-style readiness with DB + events table + Ollama checks, 503 on DB unreachable), `tests/test_ready.py` (+5 tests) | Live: 5 gunicorn procs (1 master + 4 workers), 8 concurrent requests, SIGTERM graceful |
