...
| 2 | Whisper model picker + smart picks | Performance | **P0** | **Done** (`2a96049`, `1497cd7`, then fully wired 2026-09-05) | 5-option optgroup dropdown (4 manual + 1 smart pick). **MVP2.0.7 (2026-07-15, manualTodo 2.2)**: the 2 distil-large-v3 smart picks are commented out (distil-large-v3 is English-biased and ignores the language lock). The only smart pick that remains is `local-large-turbo` (MLX Whisper Large V3 Turbo), which is the default on M-series. On non-MLX Macs, default is `base` (the recommended manual pick). `transcribe_with_backend()` dispatches by backend, with auto-fallback to `base` when MLX isn't available. **2026-09-05**: the MLX path is fully wired and runs in an **isolated subprocess** (`scripts/mlx_transcribe_worker.py`) — running MLX in-process inside gunicorn workers crashes them with SIGABRT (macOS fork-safety abort; a laptop restart does not fix it — see [`doc/mlx-fork-crash-postmortem.md`](mlx-fork-crash-postmortem.md) for the full root-cause chain). Language detection now always uses CPU `tiny` (Metal-free, fast, cached). Verified live: 30-min video transcribes in ~2 min via MLX turbo. Investigated in `doc/BlockersOrChallengers.md` §1. |
...
| 8 | Show "ready · T:..., G:..." timing next to each video's status badge (per-step) | UX | **P2** | **Done** (`ae4df7d`, `493da3d`, `4573812`, `4d0eaa3`, plus MVP2.0.8 amendment `5a05a5f`) | `doc/manualTodo.txt` [jul11] #8 + [jul14] #8. Store `transcribe_started_at` + `transcribed_at` + `generated_at`, render `ready · T:M:SS, G:M:SS` on **course page** (`app/templates/course.html:74`). MVP2.0.4 added per-step split after the [jul14] #8 follow-up (bulk-upload queue wait was inflating the duration). **MVP2.0.8 amendment (2026-07-15)**: the same `T:M:SS, G:M:SS` suffix is **intentionally NOT rendered** on the section-videos panel added in 2.0.8 (the panel is for quick context switching, not status reporting — the plain status word like `ready` / `error` / `transcribing` is enough to decide which video to click). See `doc/MVP2.0-Status.md` §22.1 for the rationale. |
