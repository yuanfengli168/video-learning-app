...
struct MaterialsPanel: View {
    ...
    private func load() async {
        // MVP0.2 followup: ALWAYS fetch the available pool. The previous
        // shortcut (`if initialSelectedIds.isEmpty { return }`) had two
        // bugs:
        //   1. If the snapshot was stale and the Mac picker had a fresh
        //      selection, iOS would never show it until the next sync.
        //   2. The user couldn't see what WAS available to pick from
        //      (the unselected pool) unless they had something already
        //      selected.
        // The extra round-trip is cheap (~10ms) and the server returns
        // the same `available` list regardless of selection.
        isLoading = true
        loadError = nil
        do {
...
struct MaterialViewerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                if isLoading {
                    ...
                    VStack(spacing: 12) {
                        ...
                        Text("Most common cause: the PDF was generated by jsPDF or is a scanned image with no text layer. Re-upload as .md/.txt, or run OCR on the PDF first.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                    }
            ...
            .toolbar {
                // MVP0.2 followup: Share button so the user can send the
                // extracted text to another app (Notes, Files, Preview,
                // etc.). PDFKit would be the "proper" PDF viewer but
                // adds ~5MB to the binary and the extracted text IS what
                // the LLM actually reads, so this is a useful
                // workaround. If the user wants the original PDF bytes,
                // they should fetch it from the Mac.
                ToolbarItem(placement: .topBarLeading) {
                    if !text.isEmpty {
                        ...
                        .accessibilityLabel("Share material text")
                    }
                }
